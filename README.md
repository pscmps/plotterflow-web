...
選択ツールでオブジェクトを選ぶと、種類を問わず編集ツール欄へ中心X/Y、幅、高さ、回転角度を表示します。幅と高さは縦横比を保って変更され、自由線・四角・円・星にも回転を適用できます。テキストとSVG素材は「詳細」から従来の専用プロパティも開けます。
...
## コントローラープロファイルと開発中モード

通常表示では、GRBL / FluidNC、STS3215 XYZ直結、Rθ Control Web互換、カスタムの各プロファイルだけを表示します。試作中のXL330、TMC2209、DRV8835、M5Stack向けプロファイルを使用するときは、設定タブで「開発中機能を表示」を有効にしてください。開発中モードでは、対象プロファイルに付随する試験用Jog、SD管理、位置センサー／アーム校正などの専用操作も利用できます。

開発中モードの選択はブラウザの`localStorage`へ保存されます。以前の版で開発プロファイルを選択済みのブラウザは、更新後の初回起動時に開発中モードを自動的に有効化し、保存済み設定を維持します。開発中モードを明示的に無効化するとき、開発プロファイルを使用中なら確認後にGRBL / FluidNCへ切り替わります。保存済みG-codeやジョブセットは削除しません。

Serial接続中、G-code送信中、Jog中、SD転送・管理中は、表示と実行状態の食い違いを避けるため開発中モードを切り替えられません。先に処理を停止してSerialを切断してください。

プロファイルを切り替えると、Baudrate、G-codeヘッダ／フッタ、ペン命令、`ok`応答timeout、停止方式、初期化・切断コマンドが関連欄とSerialタブへ反映されます。反映後も個別の値を調整でき、`カスタム（値を維持）`へ切り替えると現在値を保ったまま手動設定できます。
...
- Pen up: `G0 Z1`
- Pen down: `G1 Z0`
- Delay: すべて0秒（非対応の`G4`を出力しない）
- Footer、初期化・切断コマンド: なし
- 出力命令: `G90 / G0 / G1 / X / Y / Z / F`のみ

`G21`、`M3`、`M5`、`M17`、`M18`などは現在のControl Web parserが受け付けないため、このプロファイルでは出力しません。転送は入力欄への読み込みだけです。Tool、Backend、校正状態とプレビューを確認してから、Control Web側で手動実行してください。
...
- ジョブ開始時の原点から最初の描画位置までの空走だけは、斜め移動を避けるためX単独移動、Y単独移動の順で出力します。以後の空走と描画は通常どおりXY同時移動です。
