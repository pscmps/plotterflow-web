...
SVG変換タブ上部の「生成して実行」は、G-codeプレビューを表示したまま接続中のFluidNCへ直接送信します。G-codeタブでは `.gcode`、`.nc`、`.tap` ファイルを直接読み込めます。

設定の「リロード動作 G-code」は `reload.gcode` を初期値にしています。Serialの「リロード」ボタンと、ジョブ選択の「リロード動作（設定）」はこの共通設定を実行します。
...
SVG変換の反転をONにした場合、G-code座標には実機向けの反転を適用し、プレビューは原稿と同じ向きに戻して表示します。
